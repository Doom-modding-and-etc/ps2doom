...
//
int I_GetTime(void) { return (SDL_GetTicks() * TICRATE) / 1000; }
...
//
void I_Quit(void) {
  // Just a simple back to browser, from uLauncELF4.13 sources (TBD : check for
  // latest code) - cosmito
  __asm__ __volatile__("	li $3, 0x04;"
                       "	syscall;"
                       "	nop;");
...
void I_WaitVBL(int count) { SDL_Delay((count * 1000) / 70); }
