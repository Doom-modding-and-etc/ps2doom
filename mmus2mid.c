...
static int TWriteVarLen(MIDI *mididata, int tracknum, register ULONG value) {
  ...
  {
    if (TWriteByte(mididata, tracknum,
                   (UBYTE)(buffer & 0xff))) // insure buffer masked
      return MEMALLOC;
...
int mmus2mid(const UBYTE *mus, MIDI *mididata, UWORD division, int nocomp) {
  ...
  if (!(mididata->track[0].data = realloc(
            mididata->track[0].data, sizeof(midikey) + sizeof(miditempo) + 4)))
    return MEMALLOC;

  // key C major
  memcpy(mididata->track[0].data, midikey, sizeof(midikey));
  // tempo uS/qnote
  memcpy(mididata->track[0].data + sizeof(midikey), miditempo,
         sizeof(miditempo));
  ...
  do {
    ...
    case PLAY_NOTE:
      if (!(NewEvent =
                MidiEvent(mididata, 0x90, MIDIchannel, MIDItrack, nocomp)))
        return MEMALLOC;
    ...
    case BEND_NOTE:
      if (!(NewEvent =
                MidiEvent(mididata, 0xE0, MIDIchannel, MIDItrack, nocomp)))
        return MEMALLOC;
    ...
    case SYS_EVENT:
      if (!(NewEvent =
                MidiEvent(mididata, 0xB0, MIDIchannel, MIDItrack, nocomp)))
        return MEMALLOC;
    ...
    case CNTL_CHANGE:
      ...
      if (data) {
        ...
        if (!(NewEvent =
                  MidiEvent(mididata, 0xC0, MIDIchannel, MIDItrack, nocomp)))
          return MEMALLOC;
      }
      data = *musptr++;
      if (TWriteByte(mididata, MIDItrack, (UBYTE)(data & 0x7F)))
  ...
  for (i = 0; i < MIDI_TRACKS; i++)
    if (mididata->track[i].len) {          // killough 10/7/98: simplify code
      if (TWriteByte(mididata, i, 0x00) || // midi end of track code
          TWriteByte(mididata, i, 0xFF) || TWriteByte(mididata, i, 0x2F) ||
