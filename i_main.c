...
config_buttons_element config_buttons[] = {
    ...
    // both button names and action names as appearing at the libconfig config
    // file
...
// Store place to be built at config loading. Have the key codes for each
// button.
...
int main(int argc, char **argv) {
  ...
  **********************************************************/
  ...
  if (ret != 0)
    printf(
        "mc0 trouble... should save to other device... To implement\n"); /// TBD
...
#endif
      int nConfig_buttonsEntries =
          sizeof(config_buttons) / sizeof(config_buttons[0]);
      for (i = 0; i < nConfig_buttonsEntries; i++) {
        nj = sizeof(config_actions) / sizeof(config_actions[0]);
        for (j = 0; j < nj; j++) {
          if (strcmp(config_actions[j].name, config_buttons[i].defaultaction) ==
              0) {
  ...
  if (configLoadSuccess == 1) {
    ...
    if (x) {
      ...
      for (i = 0; i < nConfigEntries; i++) {
        sprintf(config_probestring, "%s%s", "ps2doom.controls.",
                config_buttons[i].name);
    ...
    if (use_hdd == CONFIG_TRUE) {
      ...
      if (!config_lookup_string(&cfg, config_probestring,
                                &hdd_path_to_partition)) {
    ...
    sprintf(config_probestring, "%s",
            "ps2doom.controls.switches.swap_analogsticks");
  ...
  if (use_hdd == CONFIG_TRUE) {
    ...
    //#ifdef PS2HDD
    //#ifdef PS2HDD
    // char ps2HDDPartition[] = "hdd0:+Test";                    // TBD : name
    // of partition only char folderpath[] = "abc"; // The folder path without
    // any '/'
    ...
    // todo: rewrite the hdd support maybe i should see the open ps2 loader hdd
    // support #endif
