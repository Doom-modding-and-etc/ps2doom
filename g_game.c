...
boolean G_Responder(event_t *ev) {
  ...
  case ev_joystick:
    // joybuttons[0] = ev->data1 & 1;        /// cosmito : might interefere with
    // strafe info, so it's commented joybuttons[1] = ev->data1 & 2;
    ...
    joyxside = 0;
    if ((joyxside_byjoy == 0 && joyxside_bykey == 0) ||
        (joyxside_byjoy + joyxside_bykey == 0))
      joyxside = 0;
    else {
    ...
    /// cosmito : note: in lsdldoom, for strafe support joybuttons aren't set.
    /// only this is made.
    //  case ev_joystick:
    //	joyxside = ev->data1;     /// cosmito : joyxside is a trick for doing
    //strafe in lsdldoom 	joyxmove = ev->data2; 	joyymove = ev->data3;
...
boolean G_CheckDemoStatus(void) {
  ...
  if (timingdemo) {
    ...
    // I_Error (gametic, realtics, fps, "times %i gametics in %d realtics (%f
    // fps)");
  }
