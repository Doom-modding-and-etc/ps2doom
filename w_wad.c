...
void W_InitMultipleFiles(char **filenames) {
  ...
  // I_Error ("W_InitFiles: no files found");
  {
    ...
    printf(" Put a .WAD file along with PS2Doom.elf before running it or check "
           "if HDD paths are correct at ps2doom.config.\n\n");
    // SleepThread();
  }
...
void ps2_fseek(FILE *fd, int pos, int a) { ps2_pos = pos; }
...
void *W_CacheLumpNum(int lump, int tag) {
  ...
  if ((unsigned)lump >= numlumps) {
    ...
    printf("W_CacheLumpNum: %i >= numlumps",
           lump); // cosmito  : for doom.wad, HELP2 is not found so return null
    return NULL;
  }
