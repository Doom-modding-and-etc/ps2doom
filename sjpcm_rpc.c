...
void SjPCM_Puts(char *format, ...) {
  ...
  SifCallRpc(&cd0, SJPCM_PUTS, 0, (void *)(&sbuff[0]), 252, (void *)(&sbuff[0]),
             252, 0, 0);
}
...
void SjPCM_Play() {
  ...
  SifCallRpc(&cd0, SJPCM_PLAY, 0, (void *)(&sbuff[0]), 0, (void *)(&sbuff[0]),
             0, 0, 0);
}
...
void SjPCM_Pause() {
  ...
  SifCallRpc(&cd0, SJPCM_PAUSE, 0, (void *)(&sbuff[0]), 0, (void *)(&sbuff[0]),
             0, 0, 0);
}
...
void SjPCM_Setvol(unsigned int volume) {
  ...
  SifCallRpc(&cd0, SJPCM_SETVOL, 0, (void *)(&sbuff[0]), 64,
             (void *)(&sbuff[0]), 64, 0, 0);
}
...
void SjPCM_Clearbuff() {
  ...
  SifCallRpc(&cd0, SJPCM_CLEARBUFF, 0, (void *)(&sbuff[0]), 0,
             (void *)(&sbuff[0]), 0, 0, 0);
}
...
*/
  ...
  SifCallRpc(&cd0, SJPCM_INIT, 0, (void *)(&sbuff[0]), 64, (void *)(&sbuff[0]),
             64, 0, 0);
...
int SjPCM_InitEx(int sync, int num_blocks) {
  ...
  SifCallRpc(&cd0, SJPCM_SETNUMBLOCKS, 0, (void *)(&sbuff[0]), 64,
             (void *)(&sbuff[0]), 64, 0, 0);
...
void SjPCM_Enqueue(short *left, short *right, int size, int wait) {
  ...
  sbuff[0] = size;
  SifCallRpc(&cd0, SJPCM_ENQUEUE, 0, (void *)(&sbuff[0]), 64,
             (void *)(&sbuff[0]), 64, 0, 0);
  bufpos = sbuff[3];
}
...
int SjPCM_Available() {
  if (!sjpcm_inited)
    return -1;
  SifCallRpc(&cd0, SJPCM_GETAVAIL, 0, (void *)(&sbuff[0]), 64,
             (void *)(&sbuff[0]), 64, 0, 0);
  return sbuff[3];
}
...
int SjPCM_Buffered() {
  if (!sjpcm_inited)
    return -1;
  SifCallRpc(&cd0, SJPCM_GETBUFFD, 0, (void *)(&sbuff[0]), 64,
             (void *)(&sbuff[0]), 64, 0, 0);
  return sbuff[3];
}
...
void SjPCM_SetCallback(unsigned int threshold, void (*cb)(void)) {
  if (!sjpcm_inited)
    return;
  sbuff[0] = threshold;
  SifCallRpc(&cd0, SJPCM_SETTHRESHOLD, 0, (void *)(&sbuff[0]), 64,
             (void *)(&sbuff[0]), 64, 0, 0);
  _cb = cb;
}
...
void SjPCM_Quit() {
  ...
  SifCallRpc(&cd0, SJPCM_QUIT, 0, (void *)(&sbuff[0]), 0, (void *)(&sbuff[0]),
             0, 0, 0);
  sjpcm_inited = 0;
}
