...
//
// // static const char rcsid[] = "$Id: i_unix.c,v 1.5 1997/02/03 22:45:10 b1
// Exp $";
//-----------------------------------------------------------------------------
...
int I_StartSound(int id, int vol, int sep, int pitch, int priority) {
  ...
  // channel = Mix_PlayChannel(slot, S_sfx[id].data, 0);     // S_sfx[id].data é
  // um chunk (ver linha 446 : S_sfx[i].data = Mix_QuickLoad_RAW(data,
  // lengths[i]);)

  Mix_Chunk *chunk;
  chunk = (Mix_Chunk *)S_sfx[id].data;
  channel = PlaySampleAtChannel(slot, (sint16 *)chunk->abuf, chunk->alen,
                                vol * snd_SfxVolume, 0);
...
//
void I_UpdateSound(void *unused, Uint8 *stream, int len) { return; }
...
void I_ShutdownSound(void) {
  ...
  // Mix_CloseAudio();     // ptek : we don't want to do this, since it will
  // result in a crash
  fprintf(stderr, "\n");
}
...
void I_PauseSong(int handle) {
  int mus_pause_opt =
      0; // 0 = kill music, 1 = pause, 2 = continue   // cosmito from lsdldoom
#ifdef HAVE_MIXER
