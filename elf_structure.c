...
void GetElfFilename(const char *argv0_probe, char *deviceName, char *fullPath,
                    char *elfFilename) {
  int i;
  ...
  // set fullPath to full path including '/' (ex: 'mass:directory/',
  // 'mass:directory1/directory2/', etc (no limit over depth))
  if (isFileOnRoot) {
  ...
  // set elfFilename
  if (isFileOnRoot) {
    memcpy(elfFilename, argv0_probe + twopointsindex + 1,
           lenght - twopointsindex);
  } else {
...
void elf_loader() {
  ...
  for (i = 0; i < boot_header->phnum; i++) {
    ...
    memcpy(boot_pheader[i].vaddr, boot_elf + boot_pheader[i].offset,
           boot_pheader[i].filesz);

    if (boot_pheader[i].memsz > boot_pheader[i].filesz)
      memset(boot_pheader[i].vaddr + boot_pheader[i].filesz, 0,
