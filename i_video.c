...
extern SDL_Joystick *joystick; // cosmito : shared between i_video.c and
                               // i_system.c for joystick staffing
...
void I_BeginRead(void) {
  ...
  for (y = 0; y < disk_image_h; ++y) {
    ...
    memcpy(saved_background + y * disk_image_w, screenloc, disk_image_w);
    memcpy(screenloc, disk_image + y * disk_image_w, disk_image_w);
  }

  SDL_UpdateRect(screen, screen->w - disk_image_w, screen->h - disk_image_h,
...
static void LoadDiskImage(void) {
  ...
  for (y = 0; y < disk_image_h; ++y) {
    memcpy(disk_image + disk_image_w * y, screens[0] + SCREENWIDTH * y,
           disk_image_w);
  }
...
void I_GetEvent(SDL_Event *Event) {
  ...
  case SDL_JOYBUTTONDOWN: {
    ...
    break;
    //case 10:                          /// Using dlanor suggestion : It's easy
    to knock off this button during gameplay.   // 'Analog Left click' PS2
    button
    //    event.data1 = KEY_TAB;
    break;
    ...
    if (config_buttons_int[Event->jbutton.button] != -1) {
      ...
      if (event.data1 > 255) {
        ...
        if (joyxside_bykey == STRAFELEFT)
          joyxside_bykey = -1;
        else if (joyxside_bykey == STRAFERIGHT)
          joyxside_bykey = 1;
  ...
  case SDL_JOYAXISMOTION: {
    ...
    if (Event->jaxis.axis == axis2) {
      Sint16 val = Event->jaxis.value /
                   3000; // cosmito : well, 3000 should be replaced by JOYVAL
                         // for consistency... TBD : Test later
      if (abs(val) < 3) {
        if (last_key ==
            0) // last_key also acts like a flag : when == 0, means previously
               // was instructed a strafe action, if != -0, holds the key code
        {
    ...
    // event.data1 = joyxside_byjoy;   // actually, event.data1 is not relevant
    // anymore. info for strafe is directly read at joyxside_byjoy and
    // joyxside_bykey
    event.data2 = joy_x;
    event.data3 = joy_y;
    event.type = ev_joystick;
  ...
  case SDL_JOYHATMOTION: {
    ...
    if ((((prev_jhat_value & SDL_HAT_LEFT) ^
    ...
    else if ((((prev_jhat_value & SDL_HAT_LEFT) ^
               (Event->jhat.value & SDL_HAT_LEFT)) == SDL_HAT_LEFT) &&
             ((Event->jhat.value & SDL_HAT_LEFT) == 0)) {
      if (config_buttons_int[12] != -1) {
        event.data1 = config_buttons_int[12];
        if (event.data1 > 255) {
          joyxside_bykey = 0;
    ...
    if ((((prev_jhat_value & SDL_HAT_RIGHT) ^
    ...
    } else if ((((prev_jhat_value & SDL_HAT_RIGHT) ^
                 (Event->jhat.value & SDL_HAT_RIGHT)) == SDL_HAT_RIGHT) &&
               ((Event->jhat.value & SDL_HAT_RIGHT) == 0)) {
      if (config_buttons_int[13] != -1) {
        event.data1 = config_buttons_int[13];
        if (event.data1 > 255) {
          joyxside_bykey = 0;
    ...
    if ((((prev_jhat_value & SDL_HAT_UP) ^ (Event->jhat.value & SDL_HAT_UP)) ==
         SDL_HAT_UP) &&
        ((Event->jhat.value & SDL_HAT_UP) == SDL_HAT_UP)) {
      // TBD : modifier
    ...
    } else if ((((prev_jhat_value & SDL_HAT_UP) ^
                 (Event->jhat.value & SDL_HAT_UP)) == SDL_HAT_UP) &&
    ...
    if ((((prev_jhat_value & SDL_HAT_DOWN) ^
    ...
    } else if ((((prev_jhat_value & SDL_HAT_DOWN) ^
                 (Event->jhat.value & SDL_HAT_DOWN)) == SDL_HAT_DOWN) &&
               ((Event->jhat.value & SDL_HAT_DOWN) == 0)) {
      if (config_buttons_int[15] != -1) {
        event.data1 = config_buttons_int[15];
        if (event.data1 > 255) {
          joyxside_bykey = 0;
...
void I_InitGraphics(void) {
  ...
  */
  ...
  if (multiply == 1 && !SDL_MUSTLOCK(screen)) {
    ...
    screens[0] =
        (unsigned char *)Z_Malloc(SCREENWIDTH * SCREENHEIGHT, PU_STATIC, NULL);
    if (screens[0] == NULL)
