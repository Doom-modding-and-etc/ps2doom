...
///
int PlaySampleAtChannel(int selected_channel, sint16 *sampleAddress,
                        int sampleLenght, int vol, int stereo) {
...
void Mixer_Tick() {
  ...
  float volfact = 1.0;
  sint16 sleft = 0,
         sright = 0; // TBD : should be static for better performance ?
  sint32 smixLeft = 0, smixRight = 0;
  // int allChannelsInactive = 1;                    // temporary solution for
  // status...

  // debug1++;
  // if (debug1 > 25)
  //{ printf("m_mixer_playing = %d  |  %d\n", m_mixer_playing, count); debug1 =
  //0;
  //}

  // int mixer_period = 2;       // halves frequency. 1 is no change.  TBD : put
  // as argument of Init
  ...
  for (mi = 0; mi < MIXER_BUFSIZE; mi = mi + mixer_period) {
    ...
    for (nchan = 0; nchan < _MIXER_MAXCHANNELS; nchan++) {
      ...
      if (channels[nchan].counter > 0) {
        ...
        // If end of sample was reached, put sample into inactive and reset the
        // remaining params
        if (channels[nchan].counter <= 0) {
    ...
    if (smixLeft > 0x7FFF)
      smixLeft = 0x7FFF; // limit sample (borrowed from pgen source)
    else if (smixLeft < -0x8000)
      smixLeft = -0x8000;
    mixbuffer_L[mi] = (sint16)smixLeft;

    if (smixRight > 0x7FFF)
      smixRight = 0x7FFF;
    else if (smixRight < -0x8000)
      smixRight = -0x8000;
  ...
  SjPCM_Enqueue((sint16 *)mixbuffer_L, (sint16 *)mixbuffer_R, MIXER_BUFSIZE,
                /*1*/ 0);
